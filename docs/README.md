...
В общем, есть несколько типов кнопок, которые мы используем.
Это кнопки первого, второго и третьего уровней. Не забывайте добавлять к кнопке атрибут ```aria-label="..."```
внутри него нужно описать что делает эта кнопка. И ещё кнопкой может служить как тэг ```<button>``` так и тэг ```<a>```,
никаких дивов и спанов

#### Кнопка первого уровня

Если вы хотите сделать элемент кнопкой первого уровня, то вам всего-то стоит добавить к нему класс ```.primary-btn```,
так же стоит сказать, что предусмотренно несколько видов данной кнопки: большая, средняя и маленькая. Они регулируются по высоте, ширине и размеру текста.
Для изменения типа кнопки достаточно лишь в конце класса добавить окончание `-lg`, `-md`, `-sm`. Так же к ней уже прописаны все состояния 

Внизу приведён пример как это выглядит в HTML и показана пикча с самой кнопкой:

```html
<button class="primary-button" aria-label="кнопка первого уровня">Кнопка</button>
```

![img](img/primary-btn.jpg)  

#### Кнопка второго уровня

К ней распространяется всё то же самое, что и к прошлой кнопке, только класс тут будет  ```.secondary-btn```

Внизу приведён пример как кнопка выглядит в HTML и показана пикча с самой кнопкой:

```html
<button class="secondary-btn" aria-label="кнопка второго уровня">Кнопка</button>
```

![img](img/secondary-btn.jpg)
